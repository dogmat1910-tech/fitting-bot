...
FASHN_MODE = os.getenv("FASHN_MODE", "quality")
...
WELCOME = (
    ...
    "💬 Можешь добавить <b>подпись</b> к альбому, чтобы указать тип одежды:\n"
    "   • «верх» / «топ» / «футболка»\n"
    "   • «низ» / «джинсы» / «юбка»\n"
    "   • «платье» / «комбинезон»\n"
    "Без подписи бот определит сам.\n\n"
    "⏱ Примерка занимает 10–30 секунд."
)
...
CATEGORY_KEYWORDS = {
    "tops": ("верх", "топ", "футболк", "майк", "рубашк", "блузк", "свитер", "кофт",
             "tops", "top", "shirt", "t-shirt", "blouse", "sweater"),
    "bottoms": ("низ", "джинс", "брюк", "штан", "юбк", "шорт",
                "bottoms", "bottom", "pants", "jeans", "skirt", "shorts"),
    "one-pieces": ("плать", "комбинезон", "сарафан",
                   "one-pieces", "one-piece", "dress", "jumpsuit"),
}


def _detect_category(caption: str) -> str:
    text = caption.lower()
...
async def _process_album(gid: str) -> None:
    ...
    if len(photos) < 2:
        await photos[0].answer("Нужно <b>2 фото</b> в одном альбоме: фото себя + фото вещи.")
        return

    photos.sort(key=lambda m: m.message_id)
    model_msg, garment_msg = photos[0], photos[1]

    caption = next((p.caption for p in photos if p.caption), "") or ""
    category = _detect_category(caption)

    status = await model_msg.answer(
        f"⏳ Примеряю ({category}, ~30 сек)..."
    )
    ...
    try:
        ...
        result_url = await _try_on(model_url, garment_url, category)
        ...
        await status.edit_text(
            f"❌ Не получилось: {e}\n\n"
            "Совет: укажи тип одежды в подписи к альбому, чтобы помочь модели."
        )
...
async def _try_on(model_url: str, garment_url: str, category: str) -> str:
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        pred_id = data["id"]
        log.info("FASHN prediction started: id=%s category=%s mode=%s", pred_id, category, FASHN_MODE)
...
async def main() -> None:
    log.info("Bot starting... model=%s mode=%s", FASHN_MODEL, FASHN_MODE)
